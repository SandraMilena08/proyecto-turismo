...
@Component({
  ...
  styleUrls: ['./listar-ot.component.css']
})
export class ListarOtComponent implements OnInit {
  ...
  public paginado?: Paginado<OfertaTuristica> | null; // Objeto con la información paginada
  ...
  // Redirigir a ver detalle de plan o servicio turístico
  public verDetalle(id: number) : void {
    this.router.navigateByUrl(`${ this.modulo! }/${ id }`);
  }

  public async paginar(pagina: number) : Promise<void> {    
    this.router.navigateByUrl(`${ this.modulo }/${ pagina }/12`)
  ...
  public async cargarInformacion() : Promise<void> {        
    ...
    await this.obtenerDatosPaginacion()
      .then(params => {
        ...
        if (this.modulo! == 'servicios') { // Servicios turísticos
          this.servicioService.leerPaginadoTurista(this.parametros!, pagina, cantidad)
            .then(res => {              
              this.paginado = res;  
          ...
          this.planService.leerPaginadoTurista(this.parametros!, pagina, cantidad)
            .then(res => {
              this.paginado = res;  
              if (this.paginado != null) {
                if (this.paginado!.contenido!.length > 0) // Verificar si la lista de ofertas turísticas tiene información
